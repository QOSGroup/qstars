...
func AdvertisersCmd(cdc *wire.Codec) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "Advertisers",
		Short: "Advertisers",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			tx := Advertisers(cdc, amount, privatekey, cointype, deposit, qscnonce)
			...
			result := AdvertisersBackground(cdc, string(ri.Result), 0)
