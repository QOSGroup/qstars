...
func (cstub AuctionStub) ResultNotify(ctx context.Context, txQcpResult interface{}) *types.Result {
	...
	if ok == false {
		...
		if ok {
			...
			auction = jianqian.Auction{tmpacution.Article, tmpacution.Address, tmpacution.CoinsType, tmpacution.OtherAddr, tmpacution.Amount, tmpacution.AuctionTime}
