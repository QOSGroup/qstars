...
::: tip Current Testnet
See the [testnet repo](https://github.com/QOSGroup/qstars) for 
...
```bash
qstarsd init --name <your_custom_name>
```
...
You can edit this `name` later, in the `~/.qstarsd/config/config.toml` file:
...
```bash
rm $HOME/.qstarsd/config/addrbook.json $HOME/.qstarsd/config/genesis.json
qstarsd unsafe_reset_all
```
...
```bash
cd $GOPATH/src/github.com/QOSGroup/qstars
...
Fetch the testnet's `genesis.json` file into `qstarsd`'s config directory.

```bash
mkdir -p $HOME/.qstarsd/config
curl https://raw.githubusercontent.com/cosmos/testnets/master/latest/genesis.json > $HOME/.qstarsd/config/genesis.json
```
...
Your node needs to know how to find peers. You'll need to add healthy seed nodes to `$HOME/.qstarsd/config/config.toml`. Here are some seed nodes you can use:
...
View the status of the network with the [QStars Explorer](https://explorecosmos.network). Once your full node syncs up to the current block height, you should see it appear on the [list of full nodes](https://explorecosmos.network/validators). If it doesn't show up, that's ok--the Explorer does not connect to every node.


## Upgrade to Validator Node

You now have an active full node. What's the next step? You can upgrade your full node to become a QStars Validator. The top 100 validators have the ability to propose new blocks to the QStars Hub. Continue onto [the Validator Setup](../validators/validator-setup.md).
